...
- マネージャーの采配やコミュニケーションはプレーヤーの成長、活躍に非常に大きな影響を与えることを実感。マネージャー業務として若手社員の人材育成を強く意識しながらプロジェクトを推進し、その立場でアプリケーションを遺漏なくリリースできたことに大きなやりがいを覚える
...
- Composition Root を含むアプリケーション全体の構成を整理・言語化し、Application, Domain, Infrastructure 層間の依存関係を明確化。設計方針の共通認識をチーム内に形成
  - レイヤーの責務を明確にすることで設計の一貫性を保ち、チーム内での共通理解の醸成を促進
  - テスト未導入のコードベースに対し、自動テスト導入の意義を整理、共有し、品質向上への意識改革を支援
- 自動テスト導入にあたり、各レイヤーの特性に応じたテスト方針を提示
  - ドメイン層はビジネスロジックの単体テスト、アプリケーション層はユースケース単位の結合テストとして整理
  - 影響範囲や開発優先度を踏まえ、ドメイン層から段階的にテスト導入を推進し、開発サイクルへの継続的な組み込みを支援
- 他チームと連携し、開発に関わる業務プロセス全体の改善に取り組む
- 制約理論（TOC: Theory of Constraints）を基に、企画・デザイン・実装・テストの各工程を分析し、ボトルネックの特定を試みる改善活動に参画
- 現在は、特定された制約工程のスループット向上に向けた対応策の実行段階に取り組んでいる
